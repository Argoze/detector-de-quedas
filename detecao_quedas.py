...
print("🚀 A iniciar Sistema Edge AI no Jetson Nano...")
load_dotenv()
...
if not os.path.exists(model_path):
    print("A baixar modelo MediaPipe Lite...")
...
if not cap.isOpened():
    ...
    print(f"Verifique se a câmera {fonte_de_video} está conectada ao Jetson.")
    exit()

# --- RESOLUÇÃO OTIMIZADA PARA JETSON NANO (1280x720) ---
...
while cap.isOpened():
    ...
    # Data e Hora sempre visível na tela
    agora = datetime.now().strftime("%d/%m/%Y  %H:%M:%S")
    cv2.putText(frame, agora, (20, 110), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    ...
    cv2.namedWindow('Monitoramento Assistivo - Edge AI', cv2.WINDOW_NORMAL)
    cv2.imshow('Monitoramento Assistivo - Edge AI', frame)
